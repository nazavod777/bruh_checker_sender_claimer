    ...
    async def start_work(self) -> None:
        ...
        async with aiohttp.ClientSession(headers={
            **headers,
            ...
            while True:
                try:
                    r = await bypass_errors(target_function=session.post,
                                            url=f'https://bruhcoin.co/api/sinature?userAddress={self.address}')

                    if (await r.json())['error']:
                        if (await r.json())['errorMsg'] == 'Not eligible':
                            ...
                            logger.error(f'{self.private_key} | Unexpected Response, Text: {await r.text()}')

                            async with aiofiles.open('eligible_checker_result/unexpected_response.txt', 'a',
                                                     encoding='utf-8-sig') as f:
                                await f.write(f'{self.private_key}\n')
